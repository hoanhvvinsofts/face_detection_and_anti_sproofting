...
from .faces_embedding import embedding_for_increment
...
def main(video_input_path, label_name):
    ...
    output_frames_folder = "E:/Timekeeping/Face Recognition with InsightFace/datasets/train/" + str(label_name)
    ...
    embedding_path = embedding_for_increment(output_frames_folder, embedding_path="outputs/embeddings.pickle")
    ...
    model = edit_model_for_increment(num_classes, model_path="outputs/my_model.h5")

    # 5TH STEP: Increment training with new data and label
    train_new_model(model, labels, embeddings)

    # # 5TH STEP: Test with recognize_image/stream/video.py

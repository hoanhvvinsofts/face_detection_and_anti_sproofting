...
def embedding_all():
    ...
    imagePaths = list(paths.list_images("datasets/train"))
...
def embedding_for_increment(image_folder, embedding_path=args.embeddings):
    # Load embedding pickle file as dict datatype
    ...
    imagePaths = os.listdir(image_folder)
    # Loop over the imagePaths
    for (i, imagePath) in enumerate(imagePaths):
        imagePath = image_folder + "/" + imagePath
        # extract the person name from the image path
        print("[INFO] processing image {}/{}".format(i+1, len(imagePaths)))
...
embedding_all()
# embedding_for_increment("E:/Timekeeping/Face Recognition with InsightFace/datasets/unlabel/hoan")
